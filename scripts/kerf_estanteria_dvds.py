...
OUTPUT_DIR = Path("outputs/kerf_estanteria_dvds")
OUTPUT_FILE = OUTPUT_DIR / "kerf_estanteria_dvds.json"
...
SECTION_WIDTH = int((INTERNAL_HORIZONTAL - BOARD_THICKNESS) / 2)
BACK_PANEL_WIDTH = int(OUTER_WIDTH / 2)
SHELF_LEVELS = 7
...
def place_piece(
    ...
    board["pieces"].append(
        {
            "name": source_piece["name"],
            "semantic": source_piece["semantic"],
            "x": x,
            "y": y,
            "width": width if width is not None else source_piece["width"],
            "height": height if height is not None else source_piece["height"],
            "thickness": source_piece["thickness"],
            "label": source_piece["label"],
        }
    )
    ...
    board["cuts"].append(
        {
            ...
            "y": y,
            "width": KERF_MM,
            "height": height if height is not None else board["height"],
        }
    )
...
def build_pine_boards(parts: list[dict]) -> list[dict]:
    ...
    shelves = [
        item
        for item in parts
        if item["semantic"] == "shelf"
    ]

    second_column_x = CABINET_DEPTH + KERF_MM
    right_remainder_x = (CABINET_DEPTH * 2) + (KERF_MM * 2)
    right_remainder_width = PINE_BOARD["width"] - right_remainder_x

    board = make_board("PINE BOARD 1", PINE_BOARD)

    place_piece(board, left_side, 0, 0)
    add_vertical_cut(board, CABINET_DEPTH)

    place_piece(board, right_side, second_column_x, 0)
    add_vertical_cut(board, second_column_x + CABINET_DEPTH)

    add_remainder(
        board,
        right_remainder_x,
        0,
        right_remainder_width,
        PINE_BOARD["height"],
    )
    ...
    place_piece(board, top_panel, 0, 0)
    add_vertical_cut(board, CABINET_DEPTH)

    place_piece(board, bottom_panel, second_column_x, 0)
    add_vertical_cut(board, second_column_x + CABINET_DEPTH)
    ...
    place_piece(board, center_divider, 0, 0)
    add_horizontal_cut(board, 0, DIVIDER_HEIGHT, CABINET_DEPTH)
    ...
    place_two_stacked_shelves(board, shelves[0], shelves[1], second_column_x)
    add_vertical_cut(board, second_column_x + CABINET_DEPTH)
    ...
    for board_number, chunk in enumerate(shelf_chunks, start=4):
        board = make_board(
            f"PINE BOARD {board_number}",
            PINE_BOARD,
        )

        place_two_stacked_shelves(board, chunk[0], chunk[1], 0)
        add_vertical_cut(board, CABINET_DEPTH)

        place_two_stacked_shelves(board, chunk[2], chunk[3], second_column_x)
        add_vertical_cut(board, second_column_x + CABINET_DEPTH)
...
def build_back_boards(parts: list[dict]) -> list[dict]:
    ...
    for index, back_piece in enumerate([back_left, back_right], start=1):
        ...
        place_piece(board, back_piece, 0, 0)
...
def main() -> None:
    ...
    payload = {
        "project_name": "kerf_estanteria_dvds",
        "kerf_mm": KERF_MM,
        "cabinet": {
            "type": "dvd_shelf",
